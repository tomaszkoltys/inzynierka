...
export const MyHelpOffers = () => {
  ...
  //pobierz oferty pomocy, ktore wystawił user o id = 2(wolontariusz)
  //chyba blad w bazie(nazwy na odwót) bo zapytanie /myrequests powinno zwracac oferty wystawione przez uchodzce o danym id a zwraca wystawione przez wolontariusza???
  // /myhelpoffers zwraca oferty wystawione przez uchodzce o danym id a powinno byc dla wolontariusza???
  const user_id = 2;

  useEffect(() => {
    axios
      .get<OfferProps[]>(
        `http://localhost:8080/myrequests?currentUserId=${user_id}`
      )
      .then((response) => {
